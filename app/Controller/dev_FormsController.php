...
class FormsController extends AppController {
	...
	private $email_from = array('developers@nublue.co.uk' => 'Teva Scheme Website');
    ...
    private $email_sign_up_csv = 'Ukteva.Scheme@tevauk.com';
    ...
    public function sign_up_submit() {
        if ($this->request->isPost()) {
            ...
            $msg .= '<tr style="line-height:40px;">
                <td width="200px"><strong>Ammending existing details:</strong></td>
...
Business Bank Account Name: ' . $this->request->data['bank-account-name'] . '
Bank: ' . $this->request->data['bank-name'] . '    
Account Number: ' . $this->request->data['account-number'] . '
Sort Code: ' . $this->request->data['sort-code'] . ' 
Unique ID: ' . $unique_id . '
Account details sent by post: ' . ($this->request->data['bank-confirmation']=="on"?"yes":"no").'
Details Amended: ' . ($this->request->data['amend']=="Yes"?"yes":"no");

                if ($this->emails_enabled) {
                    $this->send_bank_details($bank_msg, $unique_id);
                }
     ...
     * @return void
     */
    protected function send_bank_details($bank_msg, $unique_id) {
        // Setup mail headers.
        $headers = array('From' => $this->email_bank_from,
        ...
        openssl_pkcs7_encrypt($enc_in_file, $enc_out_file, $pubkey, $headers_msg, PKCS7_TEXT, 1);
