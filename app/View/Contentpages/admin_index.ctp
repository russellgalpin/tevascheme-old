<div class="contentpages index">
    <h2><?php echo __('Pages'); ?></h2>
    <table cellpadding="0" cellspacing="0">
        <tr>		
            <!--<th><?php echo $this->Paginator->sort('id'); ?></th>-->
            <th><?php echo $this->Paginator->sort('title'); ?></th>	
            <th><?php echo $this->Paginator->sort('section_id'); ?></th>
            <!--<th><?php echo $this->Paginator->sort('template'); ?></th>
            <th><?php echo $this->Paginator->sort('layout'); ?></th>-->
            <th class="actions"><?php echo __('Actions'); ?></th>
        </tr>
        <?php foreach ($contentpages as $contentpage): ?>
            <tr>		
                <!--<td><?php echo h($contentpage['Contentpage']['id']); ?>&nbsp;</td>-->
                <td><?php echo h($contentpage['Contentpage']['title']); ?>&nbsp;</td>
                <td><?php echo ($contentpage['Section']['title']) ? $contentpage['Section']['title'] : '<span class="unassigned">Not assigned</em>'; ?></td>
               <!--<td><?php echo h($contentpage['Contentpage']['template']); ?>&nbsp;</td>
                <td><?php echo h($contentpage['Contentpage']['layout']); ?>&nbsp;</td>                -->

                <td class="actions">			
                    <?php echo $this->Html->link(__('Edit'), array('action' => 'edit', $contentpage['Contentpage']['id'])); ?>
                    <?php echo $this->Form->postLink(__('Delete'), array('action' => 'delete', $contentpage['Contentpage']['id']), null, __('Are you sure you want to delete: %s?', $contentpage['Contentpage']['title'])); ?>
                </td>
            </tr>
        <?php endforeach; ?>
    </table>
    <p>
        <?php
        echo $this->Paginator->counter(array(
            'format' => __('Page {:page} of {:pages}')
        ));
        ?>	</p>

    <div class="paging">
        <?php
        echo $this->Paginator->prev('< ' . __('previous'), array(), null, array('class' => 'prev disabled'));
        echo $this->Paginator->numbers(array('separator' => ''));
        echo $this->Paginator->next(__('next') . ' >', array(), null, array('class' => 'next disabled'));
        ?>
    </div>
</div>
<div class="actions">
    <h3><?php echo __('Actions'); ?></h3>
    <ul>
        <li><?php echo $this->Html->link(__('New Page'), array('action' => 'add')); ?></li>		
    </ul>
</div>
